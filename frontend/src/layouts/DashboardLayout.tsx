    ...
    <div className="min-h-screen bg-panel text-ink lg:flex">
      <aside className={`flex flex-col border-b border-slate-800 bg-navy text-white shadow-xl transition-all lg:fixed lg:inset-y-0 lg:border-b-0 ${isCollapsed ? "lg:w-20" : "lg:w-72"}`}>
        ...
        <div className={`mx-3 mb-3 hidden rounded-xl border border-white/10 bg-white/[0.06] p-3 lg:block ${isCollapsed ? "px-2 text-center" : ""}`}>
          ...
          {!isCollapsed ? <p className="mt-2 inline-flex rounded-full border border-white/10 bg-white/10 px-2 py-1 text-xs font-semibold capitalize text-slate-200">{roleLabel}</p> : null}
        </div>
        <nav className="flex gap-1 overflow-x-auto px-3 pb-3 lg:block lg:flex-1 lg:space-y-1 lg:overflow-visible">
          {items.map((item) => {
        ...
        <div className="hidden border-t border-white/10 p-3 lg:block">
          <button className={`btn w-full border-white/10 bg-white/5 p-2 text-slate-200 hover:bg-white/10 ${isCollapsed ? "justify-center" : "justify-start"}`} type="button" aria-label={isCollapsed ? "Expand sidebar" : "Collapse sidebar"} onClick={() => setIsCollapsed((value) => !value)}>
        ...
        <header className="sticky top-0 z-10 border-b border-line bg-white/90 px-4 backdrop-blur lg:px-6">
          <div className="mx-auto flex h-16 max-w-[1440px] items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="hidden rounded-full border border-line bg-white px-3 py-1.5 text-xs font-semibold capitalize text-steel sm:inline-flex">
                {roleLabel}
              </span>
